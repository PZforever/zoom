...
*/
  ...
  setTime:function(timestamp,type,bol){
    ...
    if (bol) {
      ...
      Date.prototype.toLocaleString = function() {
        ...
        return this.getFullYear() + type + (this.getMonth() + 1) + type + this.getDate()+' '+ hour + ":" + minutes;
      };
    }else{
      //设置不同的格式 
      Date.prototype.toLocaleString = function() {
  ...
  */
  ...
  // 自动获取地址后从后台获取本地数据库中获取对对应省市区code,传入区的名字fn回调函数，参数为省市区code
  getCode:function(parentName,cityName,areaName,fn){
    // 向后台提供区地址，返回省市区code
    this.POST('mobile/common/getAreaCodeByName','',{'sname':parentName,'pname':cityName,'qname':areaName},fn)
  }
}
